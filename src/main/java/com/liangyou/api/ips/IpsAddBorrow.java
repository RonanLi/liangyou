...
public class IpsAddBorrow extends IpsModel implements Serializable {
	...
	public IpsAddBorrow(){
		super();
		this.setUrl(this.getSubmitUrl()+"/CreditWeb/registerSubject.aspx");
	...
	public void doIpsAddBorrowXml(String xml){
		IpsRegister cre = new IpsRegister();
		XmlTool1 tool = new XmlTool1();
		tool.SetDocument(xml);
		this.setBidNo(tool.getNodeValue("pBidNo"));
	...
	public IpsAddBorrow doReturnCreate(String xml){
		...
		XmlTool1 tool = new XmlTool1();
		tool.SetDocument(xml);
		ipsBorrow.setBidNo(tool.getNodeValue("pMerBillNo"));

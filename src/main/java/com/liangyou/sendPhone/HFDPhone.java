...
import com.liangyou.util.SingletonClient;


/**
 * @version 创建时间：2013-12-30 下午5:08:50 类说明：汇丰贷使用短信接口类
 */
public class HFDPhone {
	
	public static String password=Global.getValue("sendsms_password");// 密码,请通过亿美销售人员获取
	
	public static void init() throws Exception{
		SingletonClient.getClient().registEx(password);
	}
	
	public static String send(String phone , String content) {
		try {
			int i=SingletonClient.getClient().sendSMS(new String[] {phone}, content+"【汇丰贷】","",5);//带扩展码
			return "返回状态："+i;
		} catch (Exception e) {
			e.printStackTrace();
			return "发送短信异常";
	...
	public static void testSendSMS() {
		try {
			int i=SingletonClient.getClient().sendSMS(new String[] {"15858272881"}, "您好，您好的验证码是12【汇丰贷】","",5);//带扩展码

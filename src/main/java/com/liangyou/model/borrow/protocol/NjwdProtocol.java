 ...
 *
 */
public class NjwdProtocol extends BorrowProtocol {
	public NjwdProtocol(User user, long borrow_id, long tender_id,
	...
	protected void addPdfTable(PdfHelper pdf, Borrow b)
		...
		for (int i = 1; i < list.size() + 1; i++) {
			...
			if(b.getIsday()==1){
				cellList.add(b.getTimeLimitDay()+"天");
			}else{
			...
			if(b.getVerifyTime()!=null) {
				...
				if(b.getIsday()==1){
					Date  repayTime = DateUtils.rollDay(b.getVerifyTime(), b.getTimeLimitDay());//计算借款期限
					String endTimeStr =  sdf.format(repayTime);
					cellList.add(endTimeStr);
				}else{
					Date  repayTime = DateUtils.rollMon(b.getVerifyTime(), b.getTimeLimit());//计算借款期限
					String endTimeStr =  sdf.format(repayTime);
					cellList.add(endTimeStr);
				}

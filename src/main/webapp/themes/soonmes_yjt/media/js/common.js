...
if($.browser.msie) {
    if($.browser.version == '6.0' || $.browser.version == '7.0') {
        $('body').children('[class!="tip-browser"]').remove();
    }
}

$('.online-service').hover(function() {
    $('.list-service').stop(true,false).show('blind',{direction:'horizontal'},220);
},function() {
    $('.list-service').stop(true,false).hide('fade', 220);
});

// 判断客服qq是否在线，不在线按钮、文字变成灰色
// online是第三方js返回的数组，已经在footer.html中定义
// 方法外层的$(function(){});不可删除
$(function() {
    for(var i = 0;i < online.length;i++) {
        if(!online[i]) {
            $('.list-service ul li').addClass('offline').children().attr({title:'客服已离线，请留言！'});
        }
    }
});
...
$.fn.bannerPlay = function(options) {
    var options = $.extend({
        ...
        autoPlayInterval : 4000                     //图片切换间隔-毫秒
    }, options);
    var $bannerWrapper   = $(this);
    var $bannerItem      = $(options.bannerItem);
    var $changeBtnItem   = $(options.changeBtnItem);
    var playStyle        = options.playStyle;
    var slideDirection   = options.slideDirection;
    ...
    if(playStyle == 'fade') {
        ...
        var showImg = function(i) {
            $bannerItem.stop(true,false).eq(i).fadeIn(fadeSpeed, easing).siblings().fadeOut(fadeSpeed);
            $changeBtnItem.eq(i).addClass('hover').siblings().removeClass('hover');
        }

        // 指针移到切换按钮上时，根据按钮的下标显示对应的图片
        $changeBtnItem.hover(function() {
            var $self = $(this);
    ...
    }else if(playStyle == 'slide') {
        /*如果启用了easing.js中的动画效果，异步加载easing.js*/
        ...
        if(slideDirection == 'vertical') {
            ...
            var showImg = function() {
                if(currentIndex == $bannerItem.length) {
                    $changeBtnItem.first().addClass('hover').siblings().removeClass('hover');
                }else if(currentIndex == -1) {
                    $changeBtnItem.last().addClass('hover').siblings().removeClass('hover');
                }else {
                    $changeBtnItem.eq(currentIndex).addClass('hover').siblings().removeClass('hover');
                }
            ...
            var bannerListPosition = function() {
                ...
                $bannerList.children('li').css({float:'left',width:bannerItemWidth});
                $bannerList.css({
...
//常用js函数------------------------------------------------------------------------    
    ...
    (function($){
        ...
        var leng = (oLi.length);
                ...
                oLi.eq(0).addClass(o.hoverClass);
                setTimer = function() {
                   Timer = setInterval(function(){
                       showImg(index);
                       index++;
...
function showRemainTime(){
    $('.endtime').each(function(){
        if (parseInt($(this).attr('data-time')) < 0){
            $(this).before('<span style="color: #f00" id="out-of-date">此标已过期！</span>');
            $(this).hide();
        }
    });
}
...
$(window).bind('resize click keyup',function() {
    $('.tip-browser').css({height: $(window).height()});
    // footer如果刚好在浏览器窗口底部，footer上方的元素占居的空间高度之和加上footer占用的空间高度等于视口的高度
    // justAllEleHeight：footer刚好在底部时，footer上方的元素占居的空间高度之和
    var justAllEleHeight = $(window).height() - $('#footer').outerHeight(true);
    // footer上方的元素占居的空间高度之和
    var allEleHeight = 0;
    ...
    if($('#footer-blank').length) {
        ...
        // 如果footer上方的元素占居的空间高度之和小于"justAllEleHeight"，插入空白占位，使footer正好排列在视口底部
        if (allEleHeight < justAllEleHeight) {

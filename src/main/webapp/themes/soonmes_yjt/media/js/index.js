...
jQuery(function(){
		...
		var searchNames=["biaoType","rate","timeLimit","account","status"];
   	 ...
   	 Tab(".wh-bg",".zy-title span",".articleul-con");
   	 Tab(".list-index",".index-title span",".index-list");
					...
					function callback(){
					 ...
					 if(xmlHttp.status == 200){
					   	...
					   	var str="";
							...
							{
								var name=jsonDate[i].name;
								if(name.length>40){
									name=name.substring(0,37);
								}
								str += "<li><a target='_blank' href="+'article/detail.html?code='+o.code+jsonDate[i].jumpurl+'&id='+jsonDate[i].id+" title="+jsonDate[i].name+">"+name+"</a></li>";	
							}
						 $(obj).html(str);
					  }
					 }
...
function alertWithUrl(url, title,width,height){
	$.jBox.open("iframe:"+url, title,width,height,{ buttons: { '关闭': true},showScrolling:false,iframeScrolling:'yes'});
}
